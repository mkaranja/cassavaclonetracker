...
#' PS_plant Server Functions
#'
#' @noRd
mod_PS_plant_server <- function(id, r, location){
  moduleServer( id, function(input, output, session){
    ...
    observeEvent( input$scan_box_identity , {
      if(isTRUE(r$nav == "pencil")){
        if(isTRUE(r$pencil_tabs == "Planting")){
          if(isFALSE(is.null(location))){
            ...
            shinyjs::runjs("
                 navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } }).then(function(stream) {
                    var video = document.getElementById('PS_plant_1-video');
                    video.srcObject = stream;
                    video.play();
                    var canvas = document.getElementById('PS_plant_1-canvas');
                    ...
                    var captureInterval = setInterval(function() {
                      ...
                      if (code) {
                        ...
                        Shiny.setInputValue('PS_plant_1-qrResult', code.data);
                      }
    ...
    observeEvent( qr_code() , {
      ...
      if(length(qr_code())>0){
        ...
        dt <- load_data(glue::glue("SELECT tblBoxIdentity.box_id, tblBoxIdentity.variety, tblIntroduction.receiving_date, tblRecutCount.cut_number,
                      tblExitRp.number_of_plantlets, tblExitRP.number_sent_to_pencil_stem_block, tblExitRP.number_sent_to_field_seed_block, tblPencilPlanting.number_of_plantlets AS number_of_pencil_plants
                      FROM tblBoxIdentity
        ...
        if(nrow(dt)==0){
          ...
          status <- ifelse(is.na(dt$receiving_date), "Barcode has not been assigned for tracking of plantlets. Scan the correct barcode",
                           ifelse(!is.na(dt$receiving_date) & dt$cut_number==0, "Pending recut",
                                  ifelse(!is.na(dt$receiving_date) & dt$cut_number>0 & dt$cut_number < 4 & is.na(dt$number_sent_to_pencil_stem_block), "Pending recut number: ",
                                         ifelse(!is.na(dt$receiving_date) & dt$cut_number >= 4 & dt$number_sent_to_pencil_stem_block == 0, "Pending Exit from rapid propagation",
                                                ifelse(!is.na(dt$receiving_date) & dt$cut_number >= 4 & dt$number_sent_to_pencil_stem_block>0 & is.na(dt$number_of_pencil_plants), "Proceed",
                                                       ifelse(!is.na(dt$receiving_date) & dt$cut_number >= 4 & dt$number_sent_to_pencil_stem_block==0 & dt$number_sent_to_field_seed_block > 0, "No plantlets available for planting in pencil stem block.",
                                                              ifelse(!is.na(dt$receiving_date) & dt$cut_number >= 4 & dt$number_sent_to_pencil_stem_block>0 & !is.na(dt$number_of_pencil_plants) & dt$number_sent_to_pencil_stem_block > dt$number_of_pencil_plants, "Proceed",
                                                                      ifelse(!is.na(dt$receiving_date) & dt$cut_number >= 4 & dt$number_sent_to_pencil_stem_block>0 & !is.na(dt$number_of_pencil_plants) & dt$number_sent_to_pencil_stem_block <= dt$number_of_pencil_plants, "There are not plantlets available for planting.",
                                                                            "Don't know yet"))))))))
        }

        n <- ifelse(is.na(dt$number_of_pencil_plants), dt$number_sent_to_pencil_stem_block, (dt$number_sent_to_pencil_stem_block - dt$number_of_pencil_plants))
...
# mod_PS_plant_server("PS_plant_1")
