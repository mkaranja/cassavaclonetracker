...
#' planting Server Functions
#'
#' @noRd
mod_planting_server <- function(id, r, tab, location, tbl){
    ...
    observeEvent( qr_code() , {
      ...
      if(length(qr_code())>0){
        ...
        dt <- load_data(glue::glue("SELECT tblBoxIdentity.box_id, tblBoxIdentity.variety, tblIntroduction.receiving_date, tblRecutCount.cut_number,
                      tblExitRP.destination, tblExitRP.sent_to, tblExitRP.number_of_plantlets_sent, tblPencilPlanting.number_of_plantlets
                      FROM tblBoxIdentity
        ...
        if(nrow(dt)==0){
          ...
          status <- ifelse(is.na(dt$receiving_date), "Barcode has not been assigned for tracking of plantlets. Scan the correct barcode",
                           ifelse(!is.na(dt$receiving_date) & dt$cut_number==0, "Box is waiting the first recut",
                                  ifelse(!is.na(dt$receiving_date) & dt$cut_number>0 & dt$cut_number < 4 & is.na(dt$destination), paste("Box is recut number: ", dt$cut_number),
                                         ifelse(!is.na(dt$receiving_date) & dt$cut_number >= 4 & is.na(dt$destination), "Box is ready to exit rapid propagation",
                                                ifelse(!is.na(dt$receiving_date) & dt$cut_number >= 4 & !is.na(dt$destination) & is.na(dt$number_of_plantlets) & dt$sent_to=="Pencil Block", "Proceed",
                                                       ifelse(!is.na(dt$receiving_date) & dt$cut_number >= 4 & !is.na(dt$destination) & !is.na(dt$number_of_plantlets) & dt$sent_to=="Pencil Block" & dt$number_of_plantlets_sent > dt$number_of_plantlets, "Proceed",
                                                              ifelse(!is.na(dt$receiving_date) & dt$cut_number >= 4 & !is.na(dt$destination) & !is.na(dt$number_of_plantlets) & dt$number_of_plantlets_sent <= dt$number_of_plantlets, "There are not plantlets available for planting.",
                                                                     "Don't know yet")))))))
        }
...
# mod_planting_ui("planting_1")

## To be copied in the server
# mod_planting_server("planting_1")
